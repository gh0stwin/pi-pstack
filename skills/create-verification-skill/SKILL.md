...
- **Run:** how does the app start locally? Prefer the repo's own documented dev command (package scripts, Makefile, README quickstart). Note how the port is chosen and recorded, env vars, seed data, auth.
...
- **Launch:** the exact command that starts the app for verification, and how to tell it's ready (a log line, a port answering, a prompt). Choose the port per run — a free port, or the one the repo documents in the environment — and record the resulting base URL as run state; the readiness check and every later command use that recorded value, never a hardcoded port. Include teardown. For a short-lived CLI or TUI there is no server to keep alive: launch means build the binary (or install deps) once, then start each drive in its own isolated PTY or tmux session.
- **Doctor:** one read-only check that answers "is this instance worth driving?" — process up, right version/build, the run's recorded port owned by us, auth valid. An agent runs this first whenever anything looks off.
