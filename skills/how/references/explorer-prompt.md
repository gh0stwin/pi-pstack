...
Start by finding the relevant code. Use `find` to locate files, `grep` to find key symbols, and `read` to understand the actual implementation. Don't guess from names. Read the code.
