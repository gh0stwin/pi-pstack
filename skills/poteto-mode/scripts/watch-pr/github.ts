...
/**
 * Review-bot detection. A comment counts as a review bot when its author login
 * names a known review bot, or when its body carries the run markers these bots
 * put on their comments. Vendor-agnostic on purpose: the watcher only needs to
 * tell bot review passes apart from human threads.
 */
...
export function orderStack(
  ...
  while (byHead.has(current.baseRefName)) {
    ...
    // Two open PRs may point at each other's branches; stop on a repeat so the
    // walk cannot cycle forever (matching the up walk's `seen` guard).
