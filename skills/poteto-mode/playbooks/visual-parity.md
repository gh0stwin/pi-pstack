...
4. Verify each component against its baseline via image diff on the matching surface via the verification skill. A nonzero diff is a fail. Investigate the pixel delta. Re-check each component until the diff is zero, and do not stop at the first passing component.
