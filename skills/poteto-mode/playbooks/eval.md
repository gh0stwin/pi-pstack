...
6. **Verify the chain from the run trail, not self-report.** The `subagent` tool runs children with `--no-session`, so there is no session file to read. Each subagent result reports the files the child read and modified, parsed from its own JSON stream, plus a count of its other tool calls; the full per-call list is on the result details. Read that trail for every candidate and grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims. Do not read other projects' session files; that crosses workspace boundaries and reads private chats from unrelated projects.
