...
1. Capture a baseline trace via the matching verification skill.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent with `role: "perf-issue"`. Review the diff. Capture a post-fix trace.
