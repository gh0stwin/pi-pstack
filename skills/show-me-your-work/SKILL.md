...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript at `$PI_SESSION_FILE` from the bash tool environment. If it is unset (an ephemeral session), write a tight digest of the session and audit against that. Don't read other projects' session files; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, spawn a `subagent` call with `agent: worker` on a different model family from the one that did the work, passing either an explicit `model:` from `pi --list-models` or `role: "judgment and prose"`. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
