...
Do not modify files in the repo. Use any context tool available in your environment (e.g. a ticket tracker, chat, docs, observability, error tracker, or source control) to look up context referenced in the transcript. Read code, fetch tickets, query traces, but do not write code, edit skills, or commit. The parent agent applies edits based on your output.
...
Flag every moment the user manually supplied context the agent could have fetched itself via a context tool (ticket tracker, chat, docs, observability, error tracker, source control, analytics warehouse, CI, design tool, etc.) or another skill.
...
- Routing: the skill that owns the workflow this came up in. Extend it to call the relevant tool or sibling skill so the next agent fetches the context itself.

Examples of the pattern:
- User pastes a ticket title because the agent didn't query the ticket tracker. Routing: the relevant triage skill should query the ticket tracker first.
- User describes a flaky test the agent could have queried via an observability tool. Routing: the debugging skill should mention the observability tool.
- User links a chat thread the agent could have fetched via a chat tool. Routing: the relevant skill should mention the chat tool.
...
- `read` tool calls against any `SKILL.md` file (workspace `.pi/skills/` or `.agents/skills/`, user-level `~/.pi/agent/skills/` or `~/.agents/skills/`, or package-installed paths under `~/.pi/agent/git/`, `~/.pi/agent/npm/`, `.pi/git/`, and `.pi/npm/`)
