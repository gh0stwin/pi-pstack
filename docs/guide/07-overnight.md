...
- pi has no built-in loop command. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) makes the loop explicit: it names a wake signal (a watcher, `scripts/watch-pr/watch-pr`, a scheduled `pi -p` run) and re-checks the finish condition on events or a heartbeat.
