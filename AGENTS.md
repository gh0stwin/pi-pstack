...
- Benny's intake is opt-in through `intake.source` (`slack` or `github`) in the user's configuration; a config without `slack.cli` resolves to the GitHub path and needs no Slack. `automations/benny/runner/benny-run.ts` owns config/event validation and the `pi -p` prompt; extend its test file when the runner contract changes.
