...
- Benny's intake is opt-in through `intake.source` (`slack`, `github`, or `webhook`) in the user's configuration; a config with no Slack section (or `intake.source: github`) resolves to the GitHub path and needs no Slack, while a Slack section missing `slack.cli` or `slack.source_channel_id` still fails closed. `automations/benny/runner/benny-run.ts` owns config/event validation, the intake binding, and the `pi -p` prompt; the operational files read the binding and the contract in `automations/benny/references/intake-binding.md`; extend the runner test file when the runner contract changes.
