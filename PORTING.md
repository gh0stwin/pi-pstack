...
7. **Cursor model slugs.** `grok-4.6-fast-xhigh`, `claude-fable-5-1-thinking-max`, `gpt-5.6-sol-max`, and `claude-opus-5-thinking-xhigh` are not pi model ids. Replaced by `provider/model` ids from `pi --list-models`, with `inherit-parent` and `auto` meaning "omit `--model`". Lost: the vendor-tuned defaults the upstream author chose. The port's built-in defaults run every role on the parent session model (`inherit-parent`), so the package assumes no provider; `/skill:setup-pstack` assigns distinct models, which panel roles need for independent perspectives.
...
| `skills/poteto-mode/playbooks/orchestrate.md` | adapted | `skills/poteto-mode/playbooks/orchestrate.md` | Ported for pi: frontmatter made spec-compliant, Cursor primitives replaced, skill references rewritten to `/skill:` commands. Keeps `gt` stack surgery as the package's one deliberate Graphite exception, stated in the playbook and in the sibling forge rules. |
...
8. **`skills/setup-pstack/SKILL.md` rewritten.** It now writes `~/.pi/agent/pstack-models.json` instead of an always-applied `.mdc` rule, detects models with `pi --list-models`, supports a project-scoped `.pi/pstack-models.json` override, and points at `/pstack-models` and the `pstack_roles` tool to inspect the result. Its built-in defaults are provider-neutral (`inherit-parent` for every role), and it tells the user that panel roles need distinct models.
...
22. **Model resolution order** is the explicit `model` parameter → the `role` parameter against `pstack-models.json` → the agent's own `model` → the parent session model. `inherit-parent` and `auto` omit `--model`; every built-in role default is `inherit-parent`, so an unconfigured install resolves every role to the parent session model and assumes no provider. Panel roles need distinct configured models to supply independent perspectives.
...
40. **Model configuration is now pi model ids**, with `inherit-parent`/`auto` supported and `inherit-parent` as the built-in default for every role. The upstream placeholder slugs are gone.
...
50. **A GitLab-issues intake added.** `intake.source: gitlab` reads the issue named by the run event (`event.iid`, with an optional `event.url` that must agree and name the configured `gitlab.project`) and posts the verdict as exactly one comment through the tracker adapter, so the GitLab path needs no Slack values. `gitlab.token_env` names the environment variable that holds the token; the token value never enters YAML. The runner fails closed before `pi` starts on a missing project, token environment, or tracker adapter, and on a missing `iid`, a non-GitLab URL, an `iid`/URL disagreement, or a URL from another project. `templates/benny-gitlab-triage.yml` / `templates/benny-gitlab-reproduce.yml` wire the path: a GitLab issue webhook relays a normalized event as a `benny-gitlab-report` repository_dispatch, and the labeled-event guard fires only on the event's newly added label so a later label change cannot duplicate the verdict. The `intake.source` inference prefers Slack, then a GitLab section, then a repository section (GitHub); a config with both GitLab and repository sections is genuinely ambiguous and fails closed until the source is named, and the errors name all four intakes.
