...
- tools: for the slack intake, slack thread read and reply access through the configured slack cli; for the github intake, the issue named by the event and the tracker adapter; for the webhook intake, the adapter commands in the binding. my configured tracker adapter and my optional routing map in both cases.
