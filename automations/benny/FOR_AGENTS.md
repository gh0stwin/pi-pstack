...
i want two headless pi jobs that work together on one report source. i choose the source once: a slack channel, or github issues / the cli. slack is optional; if i do not use slack i do not want to install or configure it.

### run 1: triage issue reports

- trigger: when a new top-level report arrives in my configured source (a new message in the slack channel, or a new github issue / a `benny-run.ts --event` invocation), i want this run to start on that report and keep its original coordinates.
...
- tools: for the slack intake, slack thread read and reply access through the configured slack cli; for the github intake, the issue named by the event and the tracker adapter. my configured tracker adapter and my optional routing map in both cases.
- outcome: i want exactly one verdict where the report lives: a reply in the source thread for slack, or one comment on the source issue for github. the verdict carries `[benny:bug]`, `[benny:performance]`, or `[benny:other]`. a bug or performance marker may include the tracker url.
- boundary: for the slack intake i never want this run to post a root message in the source channel; for the github intake i never want it to open a new issue for the verdict.
...
- boundary: the same as run 1: never a slack source-channel root message, never a new issue for a verdict.
...
- i want both runs to fail closed when the configured intake's source coordinates, the tracker adapter, the verification skill, or the feature map are missing or uncertain. for the slack intake that list includes the slack cli.
...
- triage identity: `<slack user id for slack; tracker identity for github>`
...
2. ask which intake the human wants: `slack` or `github`. a human who does not use slack chooses `github` and is never asked for a slack cli or token.
