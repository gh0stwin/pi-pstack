...
Benny does not know how to start or drive every app. The user must configure one verification skill or adapter that implements this contract for the target app.

Set its skill name in `verification.skill_name`.

Set the completed user-facing feature map path in `verification.feature_map_path`. Copy and fill [`feature-map.example.md`](./feature-map.example.md) outside `.pi/automations/benny/` instead of editing the copied example.
