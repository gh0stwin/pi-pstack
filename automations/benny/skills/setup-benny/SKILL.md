...
2. For the GitHub and GitLab pairs, make each copied workflow's job-level `if:` guard match the configured label: replace `'triage'` in `.github/workflows/benny-github-triage.yml` and `.github/workflows/benny-gitlab-triage.yml` with `tracker.labels.intake`, and `'needs-repro'` in `.github/workflows/benny-github-reproduce.yml` and `.github/workflows/benny-gitlab-reproduce.yml` with `tracker.labels.needs_repro`, whenever the configured value is not the default. The guard cannot read `.pi/benny/configuration.yaml`, so a changed label that is not mirrored here silently stops the labeled-event trigger; the template headers repeat this requirement.
3. Add the repository secrets each workflow names (`PI_PROVIDER_KEY` or the provider keys pi needs, plus `BENNY_SLACK_BOT_TOKEN` only for the Slack intake when the CLI uses it). For the GitLab intake, name the repository secret after `gitlab.token_env`, and rename the `GITLAB_TOKEN` env key and its `secrets.GITLAB_TOKEN` reference in both copied GitLab workflows to that name whenever it is not `GITLAB_TOKEN`; the workflow passes the token to the tracker adapter, which reads it from the configured name. A mismatch between the workflow's env name and `gitlab.token_env` fails at verdict time, not at setup time. The webhook adapter owns its own credentials; add them to the workflow step when the adapter commands need them.
...
5. Set the copied workflow's timing to the repository: the sweep cadence (`cron` in the reproduce templates) and each job's `timeout-minutes` are adjustable to the repository's traffic and repro budget, and the shipped 15-minute cadence plus 45-minute triage and 120-minute reproduce timeouts are defaults, not limits.
6. For a local run, use [`../../runner/benny-run.ts`](../../runner/benny-run.ts):
